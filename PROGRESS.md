...
- アプリ全体のテーマカラーをビビッドな「青」から、より柔らかい「ティール（青緑）」に統一。
...
### 10. セットアップの自動化 (2025/1/5 追記)
- **目的:** 非エンジニアのユーザーでも、Vercelの環境変数などの難しい設定なしにアプリを利用開始できるようにする。
- **実装:**
    - **セットアップウィザード (/setup):** 環境変数が未設定の場合に自動的に表示される案内画面を作成。
    - **一時実行モード (LocalStorage):** ブラウザにAPIキーを一時保存し、Vercelへの設定前でもアプリの動作確認（地図表示ログイン）ができる機能を実装。
    - **リダイレクトガード:** 設定がない状態でトップページにアクセスしてもエラーにならず、ウィザードに誘導する保護機能を追加。
- **ドキュメント:**
...
### 11. 視認性と操作性の改善 (2026/1/7 追記)
- **レイヤー管理機能:** マップ上のピン（Spots）とマーキング（Areas）の表示/非表示を切り替える機能を実装。
- **マーカーサイズ最適化:** ピンのサイズを30pxから20pxに縮小し、密集時の視認性を向上。
- **モバイルレイアウト調整:** 操作ボタン類の配置を全体的に下に移動し、画面の有効活用と誤操作防止を図った。
- **セットアップウィザード改善:** Vercel環境変数設定の案内を追加し、本番運用への移行をスムーズにした。
- **README更新:** 「導入・開発ガイド」として構成を整理し、初心者向け（クイックスタート）とエンジニア向け（ローカル開発）の手順を明確に分離。

### 12. ポスティング管理機能の強化 (2026/1/8 追記)
- **枚数管理の改良:**
    - **機能:** エリア編集画面で「予定枚数」と「完了枚数」を入力・保存できるように改善。
    - **UI変更:** 従来のフリーテキストの「メモ」欄を縮小し、代わりに数値入力フィールドを新設。これにより配布計画と実績の正確な数値管理が可能に。
    - **UX向上:** エリア情報を保存した際に、自動的にポップアップウィンドウが閉じるように修正し、連続した操作を行いやすく改善。
- **バグ修正:**
    - **Firebase保存エラー:** 入力フォームが空の状態で保存すると `undefined` が送信されエラーになっていた問題を修正（`null` を許容するように型定義と送信ロジックを更新）。
    - **データ管理機能の拡張 (2026/1/8 追記):**
        - **CSVエクスポート/インポート:** スポット情報のバックアップと復元機能を実装。
            - エクセルでの可読性を向上させるため、写真データ(Base64URL)の出力を除外するオプションを追加。
            - 不正な座標データ(NaN)が含まれていた場合に自動的に除外するフィルター処理を追加。
        - **Danger Zone (一括削除):**
            - エリアおよびスポットデータを一括で削除する機能を実装。
            - 誤操作防止のため、二重の確認ダイアログ（操作確認＋バックアップ確認）を設置。
            - モーダル画面が長くなったため、スクロール表示に対応しレイアウト崩れを防止。
