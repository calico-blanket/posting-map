...
## 今後の展望 (ロードマップ)
今後の機能追加や改善計画については [ROADMAP.md](./ROADMAP.md) をご覧ください。
...
2. **エリア配布**: 
    - 画面左上の「多角形アイコン」をタップして、地図上に配るエリアを描きます。
    - エリアをタップして「予定」「完了」「中止」などのステータスを変更したり、「予定枚数」「完了枚数」を記録して「保存」します。
3. **写真登録**: 
    - 画面右下のカメラアイコンから、その場所の写真をアップロードして地図上にピン留めできます。（カメラアイコンからの撮影はできません。）
    - 建物名や「配布禁止」「注意事項」などのタグ付けが可能です。
4. **表示切り替え**: 
    - 画面右下の「レイヤーアイコン」で、エリアとスポットの表示/非表示を切り替えられます。
5. **データ管理**: 
    - 設定画面（管理者のみ）から、以下の機能が利用できます。
        - **バックアップ**: 全データをJSONファイルとして保存・復元。
        - **CSV連携**: スポット情報のCSVエクスポート・インポート。
        - **一括削除**: 開発・テスト用のデータ全消去（Danger Zone）。

## 導入・開発ガイド

このアプリは、エンジニアでない方でも簡単に導入できる「クイックスタート」と、エンジニア向けの「ローカル開発」の両方に対応しています。

### 1. クイックスタート (初心者・一般利用向け)　
※現在あまり初心者向けになっていないので随時改善中です。すみません。20260108作者
